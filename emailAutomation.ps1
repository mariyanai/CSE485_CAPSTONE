...
foreach ($User in $ExpiringUsers) {
    ...
    #If the user has an email, send reminder. If not, skip & add note in log.
    if(-not $email){
        $output += "User: $($User.SamAccountName) - Expiration Date: $($User.ExpirationDate) - No email associated with this account.`n"
        continue
    }
    else{
        #CHANGE -to $email in final implementation
        Send-Mail -from "DoNotReply.ExpiringPasswords@mesaaz.gov" -to thimblemari@gmail.com -subject $subject -body $body -expiryDate $expiryDate -fullName $fullName -days $days
        $output += "User: $($User.SamAccountName) - Expiration Date: $($User.ExpirationDate)`n"
    }
}

#CHANGE output path for task scheduling
